...
샘플 프로젝트는 client-a, client-b, 이렇게 2개의 아주 간단한 서비스로 구성되어 있습니다. 현재 샘플 프로젝트는 모두 React를 기반으로 하고 있기 때문에 여러 클라이언트 패키지에서 함께 사용하는(React, React-Dom, 등) 패키지는 루트에 설치하여 함께 사용할 수도 있지만, 독립적으로 실행되는 패키지의 의존성을 명확하게 하여 충돌하지 않도록 각각 패키지에 의존성을 명시하였습니다.
